    ...
    public class ApplicationContainer : IDisposable
    {
        ...
        private bool CallPluginAcctionWithWaitForPluginStateChange(IPlugin pluginReference, Action ipluginAction, byte timeOut = 5, bool throwException = true)
        {
            ...
            try
            {
                ...
                // Plugin State Check
                PluginState requestedInitialState = PluginState.Unknown;
                PluginState expectedTargetState = PluginState.Unknown;
                switch (ipluginAction.GetMethodInfo().Name)
                {
                    case "Start":
                        requestedInitialState = PluginState.Loaded;
                        expectedTargetState = PluginState.Running;
                        break;
                    case "Stop":
                        requestedInitialState = PluginState.Running;
                        expectedTargetState = PluginState.Loaded;
                        break;
                    case "Dispose":
                        requestedInitialState = PluginState.Loaded;
                        expectedTargetState = PluginState.Disposed;
                        break;
                    default:
                        data = new Dictionary<string, string>()
                        {
                ...
                if (pluginReference.Status.State != requestedInitialState && expectedTargetState != PluginState.Disposed)
                {
                    if (throwException)
                    {
                        data = new Dictionary<string, string>()
        ...
        private void PluginInstance_PluginStatusChanged(Guid pluginId, PluginState state)
        {
            ...
            if (plugin.Key != null)
            {
                lock (_instanceLocker)
                {
                    if (plugin.Key.WaitForThisState == state || state == PluginState.Disposed || (state == PluginState.Disposing && plugin.Key.WaitForThisState != PluginState.Disposed) || state == PluginState.Error)
                    {
        ...
        private void StartService()
        {
            try
            {
                lock (_instanceLocker)
                {
                    ...
                    if (wcfhostdescriptor.WcfConnection != null)
                    {
        ...
        #region LogThis
        public void LogThis(string message, Dictionary<string, string> data, Exception ex, LogLevel level, [CallerMemberName]string caller = "", [CallerLineNumber]int line = 0)
        {
            ...
            switch (level)
            {
                ...
                case LogLevel.Warning:
                    ...
                    e.Level = level == LogLevel.Warning ? MessageStackEntryLevel.Warning : MessageStackEntryLevel.Info;
                    lock (_infoStack)
                    {
                ...
                case LogLevel.Fatal:
                    ...
                    e.Level = level == LogLevel.Fatal ? MessageStackEntryLevel.FatalError : MessageStackEntryLevel.Error;
                    lock (_errorStack)
                    {
